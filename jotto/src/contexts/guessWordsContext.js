...
export function useGuessedWords() {
  ...
  // In this case the contxt value is an [value, setter] array for the context state.
  // useContext also subscribes to changes, and will update any time the context value updates we've memorised this so that it will only update when the guessedWords value updates
  const context = React.useContext(guessedWordsContext);

  // throw an error if the context doesn't exist -- means we aren't in a provider
  if (!context) {
    throw new Error("useGuessWords must be used within a GuessedWordsProvider");
  }
